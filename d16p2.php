...
function generate_beams(int $maxY, int $maxX) : Generator
{
	// Top
	// Bottom
	for ($x = 0; $x < $maxX; $x++)
	{
		yield [['y' => 0, 'x' => $x, 'dir' => Direction::DOWN]];
		yield [['y' => $maxY - 1, 'x' => $x, 'dir' => Direction::UP]];
	}
	...
	for ($y = 0; $y < $maxY; $y++)
	{
		yield [['y' => $y, 'x' => 0, 'dir' => DIRECTION::RIGHT]];
		yield [['y' => $y, 'x' => $maxX - 1, 'dir' => DIRECTION::LEFT]];
	}
}

$gen = generate_beams($maxY, $maxX);

foreach ($gen as $beams)
{
	$energised = [];
	$seenSplit = [];
	do
